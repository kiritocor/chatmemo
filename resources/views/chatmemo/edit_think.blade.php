...
<body>
    <div class="container">
        <div class="top">
            ...
            <p>{{ $think->created_at }}</p>
            <a class="delete-button" post-id="{{ $think->id }}">削除</a>
        </div>
        <div class="divider"></div>
        <div class="middle"　id="chat-container">
            <!-- ここにスクロール可能なコンテンツを配置 -->
            <div class="message">
                <div class="bubble right-bubble">何について考えましたか？</div>
            </div>
            <div class="message left">
                <div class="bubble">{{ $think->think_title }}</div>
            </div>
            <div class="message">
                <div class="bubble right-bubble">きっかけになった、関連するリンクとかはありますか</div>
            </div>
            <div class="message left">
...
function sendMessageToServer(editedMessage) {
 ...
 const thinkId = {{ $think->id }};
        // メッセージをサーバーに送信
        fetch(`/record/think/${thinkId}/update`, { 
            method: 'PUT',
            headers: {
